...
void RunAction::EndOfRunAction(const G4Run* aRun)
{
	...
	// set doses
	SetDoses();
    if(tetData->DoseWasOrganized()) SetEffectiveDose();
...
//	std::vector<G4int> remainder = {20, 25, 26, 27, 28, 29, 30, 31, 32, 33, 35, 36};
    std::vector<G4int> remainder = {20, 25, 26, 27, 28, 29, 31, 32, 33, 35, 36};
...
void RunAction::PrintLineExternal(std::ostream &out)
{
    ...
    out << tetData->GetCurrentFrameNo() << "\t" <<numOfEvent<<"\t"<<tetData->GetDeformT()<<"\t"<<initTimer->GetRealElapsed() << "\t"<< runTimer->GetRealElapsed()<<"\t";
