...
:voxelPhantom(_voxelPhantom)
{
	...
	cout<<"voxelNexted..."<<endl;

    voxVis = new G4VisAttributes(G4Colour(1,1,1,1));
...
ComputeMaterial(G4VPhysicalVolume* physVol, const G4int iy,
                const G4VTouchable* parentTouch)
{
    // protection for initialization and vis at idle state
    //
	
	if(parentTouch==0) return (voxelPhantom->GetVoxelMaterial(57));
	...
	//if ((voxelData == 43) && (iz == 50))
	//{
	//	physVol->GetLogicalVolume() -> SetVisAttributes(new G4VisAttributes(G4Colour(1.0, 0.0, 0.0)));
	//}
	if(voxelData==57) physVol->GetLogicalVolume()->SetVisAttributes(G4VisAttributes::GetInvisible());
	else physVol->GetLogicalVolume()->SetVisAttributes(voxVis);
	
    return voxelPhantom->GetVoxelMaterial(voxelData);
	//return voxelPhantom->GetVoxelMaterial(57);
...
{
	//cout<<"number of materials: " << voxelPhantom->GetVoxelMaterialSize()<<endl;
    return voxelPhantom->GetVoxelMaterialSize();
}

G4Material* VoxelNestedParameterisation::GetMaterial(G4int i) const
{
	int VoxID = voxelPhantom->GetMaterialIndex(i);
	//cout<<"Material: " << i << "VoxID: "<< VoxID <<endl;
	return voxelPhantom->GetVoxelMaterial(VoxID);
...
ComputeTransformation(const G4int copyNo, G4VPhysicalVolume* physVol) const
{
    ...
    physVol->SetTranslation(G4ThreeVector(0.,(2.*static_cast<double>(copyNo)+1.)*fVoxelHalfLengthY - fVoxelHalfLengthY*fNy, 0.));
}
