...
class Dual_quat_cu {
    ...
    {
        double w = -0.5f*( t.x * q.i() + t.y * q.j() + t.z * q.k());
        double i =  0.5f*( t.x * q.w() + t.y * q.k() - t.z * q.j());
        double j =  0.5f*(-t.x * q.k() + t.y * q.w() + t.z * q.i());
        double k =  0.5f*( t.x * q.j() - t.y * q.i() + t.z * q.w());
    ...
    Dual_quat_cu operator*(double scalar) const
    {
