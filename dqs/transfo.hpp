...
/**
  ...
  The 4x4 matrix is stored linearly with an array of 16 doubles.
...
struct Transfo {
    ...
    ///     glMultMatrixf( (GLdouble)(tr.transpose().m) );
    /// @endcode
    double m[16];
    ...
    inline Transfo operator*(double x) const {
        Transfo res;
        for(int i = 0; i < 16; i++){
            res[i] = m[i] * x;
        }
    ...
    static inline
    double MINOR(const Transfo& m, const int r0, const int r1, const int r2, const int c0, const int c1, const int c2) {
    ...
    inline bool is_frame_ortho(double eps = 0.0001f) const {
    ...
    static inline Transfo translate(double dx, double dy, double dz){
    ...
    static inline Transfo scale(double sx, double sy, double sz){
    ...
    static inline Transfo scale(double s){
    ...
    /// scale origin 'center'
    static inline Transfo scale(const Vec3& center, double s)
    {
    ...
    static inline Transfo rotate(const Vec3& axis, double angle)
    {
