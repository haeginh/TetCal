...
--------------------------
  ...
  A Quaternion is defined using four doubleing point values |x y z w|.
...
------------------------------------------------------
  ...
  -------------------------------------------------------------------
  Qdouble quaternion_magnitude( QUAT *qa )
    {
...
-------------------------------------------------------------------
  ...
  quaternion_from_euler( QUATERNION *q, Vdouble ax, Vdouble ay, Vdouble az )
  {
  ...
  http://vered.rose.utoronto.ca/people/david_dir/GEMS/GEMS.html
   ...
   //Pitch->X axis, Yaw->Y axis, Roll->Z axis
    Quaternion::Quaternion(double fPitch, double fYaw, double fRoll)
    {
       const double fSinPitch(sin(fPitch*0.5F));
       const double fCosPitch(cos(fPitch*0.5F));
       const double fSinYaw(sin(fYaw*0.5F));
       const double fCosYaw(cos(fYaw*0.5F));
       const double fSinRoll(sin(fRoll*0.5F));
       const double fCosRoll(cos(fRoll*0.5F));
  ...
  public static void setQ( Quat4f q, double pitch, double yaw, double roll )
  {
      Quat4f qx = new Quat4f((double) Math.cos(pitch/2f),
                 (double) Math.sin(pitch/2f), 0, 0);
      Quat4f qy = new Quat4f((double) Math.cos(yaw/2f), 0,
                 (double) Math.sin(yaw/2f),0);
      Quat4f qz = new Quat4f((double) Math.cos(roll/2f), 0, 0,
                 (double) Math.sin(roll/2f) );
...
class Quat_cu{
    ...
    /// 'angle' in radians
    Quat_cu(const Vec3& axis, double angle)
    {
        Vec3 vec_axis = axis.normalized();
        double sin_a = sin( angle * 0.5f );
        double cos_a = cos( angle * 0.5f );
        coeff[0]    = cos_a;
    ...
    // angle 'angle'
    // Quat_cu(const Vec3& vec, double angle)
    ...
    Mat3 to_matrix3()
    {
        double W = coeff[0], X = -coeff[1], Y = -coeff[2], Z = -coeff[3];
        double xx = X * X, xy = X * Y, xz = X * Z, xw = X * W;
        double yy = Y * Y, yz = Y * Z, yw = Y * W, zz = Z * Z;
        double zw = Z * W;
        Mat3 mat = Mat3(
    ...
    double dot(const Quat_cu& q){
        return w() * q.w() + i() * q.i() + j() * q.j() + k() * q.k();
    }

    double w() const { return coeff[0]; }
    double i() const { return coeff[1]; }
    double j() const { return coeff[2]; }
    double k() const { return coeff[3]; }
    ...
    Quat_cu operator/= (double scalar){
        coeff[0] /= scalar;
        coeff[1] /= scalar;
        coeff[2] /= scalar;
        coeff[3] /= scalar;
