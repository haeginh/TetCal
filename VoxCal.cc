...
void PrintUsage(){
	G4cerr<< "Usage: ./VoxCal -m [MACRO] -o [OUTPUT] -p [phantom name]"  <<G4endl;
	G4cerr<< "Example: ./VoxCal -m sample.in -o run.out -p ./phantoms/00M" <<G4endl;
}
...
//		G4cerr<<"ERROR: Interactive mode is not available. Please provide macro file."<<G4endl;
	...
	//
	VOXModelImport* tetData = new VOXModelImport(phantomName);

	// Set mandatory initialisation classes
	//
	// detector construction
	runManager->SetUserInitialization(new VOXDetectorConstruction(tetData));
	// physics list
