...
EXE="./build/TetCal"
OUT_DIR="importanceTest_weighted"
SHIELD_MACRO_DIR="${OUT_DIR}/shieldMacros"
EYE_MACRO_DIR="${OUT_DIR}/eyeMacros"
POST_EYE_MACRO_DIR="${OUT_DIR}/postEyeMacros"
mkdir -p "${OUT_DIR}" "${SHIELD_MACRO_DIR}" "${EYE_MACRO_DIR}" "${POST_EYE_MACRO_DIR}"
...
make_eye_macro_without_shield() {
  local eye_macro="$1"
  local out_macro="$2"
  force_one_iteration_macro "${eye_macro}" "${out_macro}"
}
...
run_shield_tune() {
  local case_name="$1"
  local base_macro="TetCal/batch_cases/${case_name}_shield_tune_TetCal_NM.in"
  local run_macro_path="${SHIELD_MACRO_DIR}/${case_name}_shield_tune_TetCal_NM.in"

  if [[ ! -f "${base_macro}" ]]; then
    echo "Shield tune macro not found: ${WORK_DIR}/${base_macro}" >&2
    exit 1
  fi

  force_one_iteration_macro "${base_macro}" "${run_macro_path}"
  run_macro "${run_macro_path}" "${case_name}_shield_tune"
}
...
run_eye_tune() {
  ...
  local macro_dir="${EYE_MACRO_DIR}"
  local max_iterations=10
  local current_macro="${macro_dir}/${case_name}_eye_iter1_TetCal_NM.in"

  if [[ ! -f "${base_macro}" ]]; then
    echo "Eye tune macro not found: ${WORK_DIR}/${base_macro}" >&2
    exit 1
  fi

  if [[ -n "${shield_case}" ]]; then
    local shield_tuned="${SHIELD_MACRO_DIR}/${shield_case}_shield_tune_TetCal_NM_tuned.in"
    if [[ ! -f "${shield_tuned}" ]]; then
      echo "Shield tuned macro not found: ${WORK_DIR}/${shield_tuned}" >&2
      exit 1
    fi
    make_eye_macro_with_shield_value "${base_macro}" "${shield_tuned}" "${current_macro}"
  else
    make_eye_macro_without_shield "${base_macro}" "${current_macro}"
  fi
  ...
  for (( iteration = 1; iteration <= max_iterations; iteration++ )); do
    ...
    local next_macro="${macro_dir}/${case_name}_eye_iter$((iteration + 1))_TetCal_NM.in"
...
run_post_eye_tune() {
  ...
  local base_macro="TetCal/batch_cases/${case_name}_eye_tune1_TetCal_NM.in"
  local max_iterations=10
  local current_macro="${POST_EYE_MACRO_DIR}/${case_name}_eye_iter1_TetCal_NM.in"

  if [[ ! -f "${base_macro}" ]]; then
    echo "Postinjection eye tune macro not found: ${WORK_DIR}/${base_macro}" >&2
    exit 1
  fi
  ...
  for (( iteration = 1; iteration <= max_iterations; iteration++ )); do
    ...
    local next_macro="${POST_EYE_MACRO_DIR}/${case_name}_eye_iter$((iteration + 1))_TetCal_NM.in"
    cp "${tuned_macro}" "${next_macro}"
    current_macro="${next_macro}"
  done
}
...
for case_name in "${shield_cases[@]}"; do
  run_shield_tune "${case_name}"
done

run_eye_tune "Preinjection_I-131_1" "Preinjection_I-131_1"
run_eye_tune "Preinjection_I-131_2" "Preinjection_I-131_2"
...
run_post_eye_tune "Postinjection_I-131_1"
run_post_eye_tune "Postinjection_I-131_2"
run_post_eye_tune "Postinjection_Tc-99m_1"
run_post_eye_tune "Postinjection_Tc-99m_2"
run_post_eye_tune "Postinjection_F-18_1"
run_post_eye_tune "Postinjection_F-18_2"
